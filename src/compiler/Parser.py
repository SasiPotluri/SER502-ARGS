...
class Parser:
	...
	def statements(self):
		...
		while self.current_tok.type == lexer.TT_NEWLINE:
	...
	def statement(self):
		...
		if self.current_tok.matches(lexer.TT_KEYWORD, 'RETURN'):
			res.register_advancement()
			self.advance()
		...
		if self.current_tok.matches(lexer.TT_KEYWORD, 'CONTINUE'):
			res.register_advancement()
			self.advance()
			return res.success(nodes.ContinueNode(pos_start, self.current_tok.pos_start.copy()))
	
		if self.current_tok.matches(lexer.TT_KEYWORD, 'BREAK'):
			res.register_advancement()
			self.advance()
		...
		if res.error:
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected 'RETURN', 'CONTINUE', 'BREAK', 'VAR', 'IF', 'FOR', 'WHEN', 'FUN', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
			))
	...
	def if_expr_c(self):
		...
		if self.current_tok.matches(lexer.TT_KEYWORD, 'ELSE'):
			res.register_advancement()
			self.advance()

			if self.current_tok.type == lexer.TT_NEWLINE:
				res.register_advancement()
				self.advance()
				...
				if self.current_tok.matches(lexer.TT_KEYWORD, 'END'):
					res.register_advancement()
					self.advance()
				else:
					return res.failure(errorclass.InvalidSyntaxError(
	...
	def if_expr_b_or_c(self):
		...
		if self.current_tok.matches(lexer.TT_KEYWORD, 'ELIF'):
	...
	def if_expr_cases(self, case_keyword):
		...
		if not self.current_tok.matches(lexer.TT_KEYWORD, case_keyword):
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
		...
		if not self.current_tok.matches(lexer.TT_KEYWORD, 'THEN'):
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'THEN'"
				))
	...
	def ternary_expr(self):
		...
		if not self.current_tok.matches(lexer.TT_KEYWORD, 'TERNARY'):
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'TERNARY'"
			))
		...
		if self.current_tok.matches(lexer.TT_KEYWORD, '::'):
			res.register_advancement()
			self.advance()
		...
		return res.success(nodes.TernaryNode(cases, else_case))

	def for_expr(self):
		res = ParseResult()

		if not self.current_tok.matches(lexer.TT_KEYWORD, 'FOR'):
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'FOR'"
			))
		...
		if not self.current_tok.matches(lexer.TT_KEYWORD, 'TO'):
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'TO'"
			))
		...
		if self.current_tok.matches(lexer.TT_KEYWORD, 'STEP'):
			res.register_advancement()
			self.advance()
		...
		if self.current_tok.type == lexer.TT_NEWLINE:
			...
			if not self.current_tok.matches(lexer.TT_KEYWORD, 'END'):
				return res.failure(errorclass.InvalidSyntaxError(
					self.current_tok.pos_start, self.current_tok.pos_end,
					f"Expected 'END'"
					))
	...
	def when_expr(self):
		...
		if not self.current_tok.matches(lexer.TT_KEYWORD, 'WHEN'):
		...
		if self.current_tok.type == lexer.TT_NEWLINE:
			...
			return res.success(nodes.WhenNode(condition, body, True))
		
		body = res.register(self.statement())
		if res.error: return res

		return res.success(nodes.WhenNode(condition, body, False))
		...
		if self.current_tok.type == lexer.TT_LPAREN:
			...
			if self.current_tok.type == lexer.TT_RPAREN:
				...
				if res.error:
					return res.failure(errorclass.InvalidSyntaxError(
						self.current_tok.pos_start, self.current_tok.pos_end,
						"Expected ')', 'VAR', 'IF', 'FOR', 'WHEN', 'FUN', int, float, identifier, '+', '-', '(' or 'NOT'"
					))
	...
	def atom(self):
		...
		elif tok.type in (lexer.TT_STRING):
			res.register_advancement()
			self.advance()
			return res.success(nodes.StringNode(tok))
		elif tok.type in (lexer.TT_BOOL):
			res.register_advancement()
			self.advance()
		...
		elif tok.matches(lexer.TT_KEYWORD, 'IF'):
			if_expr = res.register(self.if_expr())
			if res.error: return res
			return res.success(if_expr)

		elif tok.matches(lexer.TT_KEYWORD, 'TERNARY'):
			ternary_expr = res.register(self.ternary_expr())
			if res.error: return res
			return res.success(ternary_expr)

		elif tok.matches(lexer.TT_KEYWORD, 'FOR'):
			for_expr = res.register(self.for_expr())
			if res.error: return res
			return res.success(for_expr)

		elif tok.matches(lexer.TT_KEYWORD, 'WHEN'):
			when_expr = res.register(self.when_expr())
			if res.error: return res
			return res.success(when_expr)		
		
		elif tok.matches(lexer.TT_KEYWORD, 'FUN'):
		...
		return res.failure(errorclass.InvalidSyntaxError(
			...
			"Expected int, float, identifier, '+', '-', '(', '[', IF', 'FOR', 'WHEN', 'FUN'"
		))


	def power(self):
		return self.bin_op(self.call, (lexer.TT_POW, lexer.TT_MUL ), self.factor)
	...
	def term(self):
		return self.bin_op(self.factor, (lexer.TT_MUL, lexer.TT_DIV, lexer.TT_MOD))
	...
	def comp_expr(self):
		...
		if res.error:
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected int, float, identifier, '+', '-', '(', '[', 'IF', 'FOR', 'WHEN', 'FUN' or '##'"
				))
	...
	def expr(self):
		...
		if self.current_tok.matches(lexer.TT_KEYWORD, 'VAR'):
			res.register_advancement()
			self.advance()
		...
		node = res.register(self.bin_op(self.comp_expr, ((lexer.TT_KEYWORD, "&&"), (lexer.TT_KEYWORD, "||"))))

		if res.error:
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected 'VAR', 'IF', 'FOR', 'WHEN', 'FUN', int, float, identifier, '+', '~', '(' or '##'"
			))
	...
	def func_def(self):
		...
		if not self.current_tok.matches(lexer.TT_KEYWORD, 'FUN'):
			return res.failure(errorclass.InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'FUN'"
			))
	...
	def bin_op(self, func_a, ops, func_b=None):
        ...
        or (self.current_tok.type == lexer.TT_MUL and self.current_tok.value == '*')
        or (self.current_tok.type == lexer.TT_POW and self.current_tok.value == '*')
        or (self.current_tok.type == lexer.TT_DIV and self.current_tok.value == '/') 
